...
> **You are on `lightweight-portable`.** See [Branches](#branches) below for what
> that means and why it is the recommended starting point.
...
- **[`docs/lightweight-portable/architecture_report_lightweight_full.md`](docs/lightweight-portable/architecture_report_lightweight_full.md)** -
  the design document (also exported as
  [`architecture_report_lightweight_full.pdf`](docs/lightweight-portable/architecture_report_lightweight_full.pdf)):
  architectural approach, sequence diagrams for both assessed modules, how each
  requirement is met, and what the portability constraint costs, measured.
  [`docs/main/`](docs/main/) carries the same document for `main`, and
...
- **[`single_file_transfer.py`](single_file_transfer.py)** - how a single file's
  transfer is managed: fixed-size chunking, dedup, resumable upload, and an
  AIMD upload-concurrency controller. Symlinked from
...
2. **Bandwidth** - four stacked reductions: unchanged files cost a `stat` and no
   ...
   first; the residue is compressed. Measured: a 50 KB edit to a 3 MB file put
   **262,230 bytes** on the wire - one chunk, 8.3 % of the file - and a rename
   put **zero**.
3. **Reliability** - every step is idempotent or resumes from persisted state.
   Measured: a 6 MB file (24 chunks) interrupted after 3 chunks sent exactly 21
   more on the resumed pass, with the byte totals closing exactly. Multi-server
   failover, failback, and an adaptive concurrency window keep a degraded link
   converging instead of flapping.
