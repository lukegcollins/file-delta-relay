...
def stat(size=100, mtime_ns=NOW - HOUR):
    """Stand-in for os.stat_result with the two fields classify() reads."""
...
def rec(state="synced", size=100, mtime_ns=NOW - HOUR, verified_at_ns=NOW - HOUR + 1):
    """A StateDB record; by default a clean sync verified just after mtime."""
    return FileRec((size, mtime_ns), state, b"h", None, verified_at_ns)


class ClassifyTests(unittest.TestCase):
    """One test per branch of classify(), including the guard-window boundary."""
...
class ContainsTests(unittest.TestCase):
    """_contains() is the other pure helper scan()/check() build on: does a
    path fall under a root. Regression coverage for the degenerate root ==
    '/' case, where a naive `path.startswith(root + os.sep)` silently
    matches nothing (os.path.abspath('/') + os.sep == '//')."""
