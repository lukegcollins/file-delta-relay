...
def free_port() -> int:
    """Return a TCP port nothing is listening on right now."""
...
def main() -> None:
    ...
    try:
        ...
        def server_bytes(path):
            """Return the server's copy of one path, or None if tombstoned."""
            r = requests.get(f"{base}/v1/file", params={"path": path}, verify=verify)
            return r.content if r.ok else None

        def chunk_count():
            """Return how many chunks the server has stored."""
        ...
        def flaky_put(h, data):
            """Put one chunk, then refuse every put after the third."""
            with lock:
                if sent["n"] >= 3:
        ...
        # chunk_manifest() special-cases size==0 rather than open()+read()
        # a zero-byte file through the normal chunking loop.
        ...
        def flaky_sync_file(db, srv, path, **kwargs):
            """Sync one file, raising for the one path chosen to fail."""
            if path == bad:
                raise RuntimeError("simulated unexpected per-file failure")
            return real_sync_file(db, srv, path, **kwargs)
