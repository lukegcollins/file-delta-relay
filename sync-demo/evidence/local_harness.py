...
def free_port() -> int:
    """Return a TCP port number the kernel has just confirmed is free."""
...
class Recorder:
    """Collect timestamped events and per-operation byte counters."""
    ...
    def now(self) -> float:
        """Return the seconds elapsed since the recorder was created."""
        return time.time() - self.t0

    def event(self, type_: str, **fields) -> None:
        """Append one timestamped event of the given type."""
        self.events.append({"t": self.now(), "type": type_, **fields})

    def begin_op(self, name: str) -> None:
        """Start a named operation and reset its byte and chunk counters."""
        self._current_op = name
        self._op_bytes_uploaded = 0
        self._op_chunks_uploaded = 0

    def record_upload(self, nbytes: int) -> None:
        """Count one uploaded chunk against the operation in progress."""
    ...
    def end_op(self, *, logical_bytes: int) -> None:
        """Close the current operation and file its bandwidth row.

        logical_bytes is the on-disk size the operation covered, which the
        plot generator compares against the bytes actually uploaded.
        """
...
def main() -> None:
    """Run both evidence parts and write evidence/local_metrics.json."""
    rec = Recorder()
