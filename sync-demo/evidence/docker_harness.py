...
def health(url: str) -> bool:
    """Return True if the server at url answers /v1/health with 200."""
    try:
...
def has_file(url: str, local_path: str) -> bool:
    """Return True if the server holds this file byte-for-byte.

    The comparison is against the local copy's full contents, so a partially
    stored or stale file counts as absent.
    """
...
def which_server(local_path: str) -> str | None:
    """Return the name of the server holding the file, or None if neither does."""
...
def wait_for(local_path: str, timeout: float) -> tuple[str | None, float]:
    """Poll until the file lands on either server.

    Return (server, elapsed), or (None, timeout) if it never arrives.
    """
    start = time.time()
...
def make_file(name: str, size: int) -> str:
    """Write a file of random bytes into the sync root and return its path."""
...
class HealthPoller:
    """Sample /v1/health on both servers at a fixed rate from a background thread."""
    ...
    def now(self) -> float:
        """Return the seconds elapsed since the poller was created."""
...
def failover_sequence() -> dict:
    """Stop the primary mid-run, then confirm failover and failback.

    Confirm the client keeps syncing via the secondary, restart the primary,
    confirm new work returns to it -- while a background thread samples both
    servers' health for the timeline plot.
    """
    poller = HealthPoller()
    poller.start()
    events: list[dict] = []
...
def network_sweep() -> list[dict]:
    """Measure completion time vs. netem loss/delay, on a fixed-size file each time."""
...
def main() -> None:
    """Run all three measurements and write evidence/docker_metrics.json."""
