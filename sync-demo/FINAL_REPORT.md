# file-delta-relay / sync-demo - Final Report (`main`)
...
> **Which branch this is.** `main` is the throughput-oriented implementation:
> `fastcdc` content-defined chunking, BLAKE3 hashing, zstd compression, and a
> static pool of 8 upload workers. Branch `lightweight-portable` implements the
> same protocol using only the Python standard library, trading shift-resistant
> deduplication for a zero-compiled-dependency install. Neither is a
...
`single_file_transfer.py` (content-defined chunking, dedup, resumable upload) -
driven by an HTTP transport (`transport.py`) with multi-server failover. The
server (`server/app.py`) is a deliberately small content-addressed chunk store
that still enforces the two invariants correctness depends on: verify-on-write
(BLAKE3, recomputed server-side) and reference-validated commits.
...
The most compact piece of evidence for what this branch's dependencies buy is
§8.1: inserting 50 KiB at the front of an 8 MiB file - an edit that shifts every
following byte - re-sends **one chunk, 3 % of the file**, where fixed-size
chunking must re-send **100 %**.
...
│   ├── single_file_transfer.py    (FastCDC chunking, dedup, resumable upload)
...
- **`chunk_manifest()` / `sync_file()`** (`single_file_transfer.py`) turn a
  changed file into content-defined chunks (FastCDC, 64 KiB min / 256 KiB avg /
  1 MiB max, BLAKE3 per chunk), diff against the client's own last-synced
  baseline, ask the server what it is still missing, upload only that, and
  commit. A manifest holds `(offset, length, hash)` only - never chunk bytes -
  so resident file data is bounded at `WORKERS × MAX_CHUNK`, 8 MiB, regardless
  of file size.
...
- **`server/app.py`** is a content-addressed store: chunks live at their
  BLAKE3 hex digest, so puts are idempotent and dedup is set membership;
...
the next with no delay. After three consecutive failures an endpoint is marked
down and skipped until an active `GET /api/v1/status` probe says it is back; a
downed higher-priority endpoint is re-probed every
...
| **Bandwidth** | FastCDC content-defined chunking + BLAKE3 content addressing + server-side dedup (`/api/v1/collect`) + local pre-diff against the last synced manifest (`prev_hashes`) + per-chunk zstd. | A rename uploaded **0 bytes** while "touching" all 3 MB. A 50 KiB insertion at the front of an 8 MiB file re-sent **1 chunk of 33** (§8.1) - the property the compiled dependencies exist to provide. Integration checks 3, 4; **Plot 2**. |
| **Reliability** | Idempotent content-addressed puts + persisted `chunked` state (resume without re-chunking) + bounded jittered per-chunk retry/backoff + GC-race-aware commit retry + multi-server failover/failback + per-file exception isolation. | Integration checks 5, 6, 8, 9, 11; scenarios 2 and 3 (real `tc`/`netem`: 30 % loss, a 6 s total outage, a 60 s blackout - all converged); **Plots 3, 4, 5**. §6.3's byte accounting. |
| **Integrity** | Per-chunk BLAKE3 verified server-side on *every* write (409 on mismatch); file identity = BLAKE3 of the ordered chunk hashes, recomputed and checked on commit; commit validates every reference; a transient scan error is never mistaken for a deletion. | Integration checks 1, 3, 10; scenario 1 step 4 and scenario 4 both verified byte-identical server copies. |
...
| **Per-file endpoint spreading** | `run_once()` assigns each file to a *randomly chosen* healthy endpoint rather than always the priority-1 server. | `client/transport.py`; **Plot 6** - 20 real syncs against the live stack split **9 primary / 11 secondary**. |
...
.venv/bin/python evidence/chunking_shift_test.py   # the §8.1 chunker benchmark
...
5. resume ok: dropped after 3 chunks (18 stored), reconnected and finished
...
Exact chunk counts in checks 3 and 5 vary slightly between runs on this branch,
and that is a property of what is being tested rather than flakiness: every test
file is filled with fresh `os.urandom` content, so FastCDC's content-defined
boundaries land in different places each time. The invariant under test is not a
specific number but a relationship - check 3's edit always stores a small,
bounded number of chunks rather than re-uploading the unaffected majority of the
file, and check 4's rename always stores exactly zero.

### 6.2 Bandwidth, measured directly (`evidence/local_harness.py`)

| Operation | Logical bytes touched | Bytes on the wire | Ratio |
|---|---:|---:|---|
| Initial sync (2 files, 3.15 MB) | 3,151,728 | 3,145,983 | ~1.00 - nothing to dedup against yet |
| No-op pass (nothing changed) | 0 | **0** | stat only; no file was read |
| 50 KB edit to a 3 MB file | 3,145,728 | 550,146 | 17.5 % - the two chunks the edit straddled |
| Rename (no content change) | 3,145,728 | **0** | metadata-only commit |
| Delete | 0 | **0** | tombstone only |

Server-side chunk count over the same sequence: **16** after the initial sync of
both files, **18** after the edit, 18 after the rename (zero new), and 18 after
the delete.

The edit figure is the one to read carefully. 550,146 bytes is *two* chunks,
because the 50 KB edit happened to straddle a content-defined boundary. On
`lightweight-portable` the same edit cost one fixed 256 KiB chunk. That is not
evidence that fixed-size chunking is better - it is evidence that for an
*overwrite*, which strategy wins is decided by where the boundary happens to
fall. The systematic difference between the two appears only under a *shifting*
edit, which §8.1 measures directly.

### 6.3 Resume, accounted to the byte

| | Bytes uploaded |
|---|---:|
| Pass 1 - dropped after 3 chunks | 568,006 |
| Pass 2 - resumed from persisted state | 5,723,882 |
| **Total** | **6,291,888** |

The file is 6,291,456 bytes. The 432-byte excess is zstd frame overhead on
incompressible random content, so the totals close: every chunk was sent once and
only once. The resumed pass re-chunked nothing, re-read nothing it had already
sent, and the client never had to be told where it left off - it re-asked the
same set-difference question and the answer was exactly what it still owed.
...
- **Scenario 1**: creating `blob.bin` (400 KB) and `notes.txt` (6 KB) raised the
  combined chunk count to 7. Appending a line to `notes.txt` added exactly **1**
  chunk. Renaming `blob.bin` added 1 - not new content, but its existing chunk
  hash re-homed onto the other, non-replicating store, which the script reports
  explicitly rather than as dedup failure. Deleting `notes.txt` incremented the
...
- **Scenario 2**: a 12 MB file synced over a 30 %-loss/100 ms-delay link,
  ...
  16 polling checks - 71 new chunks this run, **none stored twice**.
- **Scenario 3**: failover to the secondary succeeded the moment the primary was
  ...
  over a 10 %-loss/100 ms-delay link in **28 s**; a 6 MB file survived a full
...
| 4 | `single_file_transfer.chunk_manifest()` | FastCDC's backend `mmap`s the input, and `mmap` refuses a zero-length file, so `touch empty.txt` in a synced root raised an uncaught `ValueError`. Combined with #5's missing isolation, one empty file could take down the daemon. | `chunk_manifest()` special-cases `size == 0`: no chunks, file identity = BLAKE3 of the empty byte string, which is what the general formula yields from an empty chunk list, so no protocol change. | Integration check 9. |
...
|---|---|---|---|
| 7 | `transport.HttpServer._pick` / `refresh` | `ServerEndpoint.last_probe` was read and written outside its lock. With an 8-worker upload pool, several threads could observe the same stale timestamp and all fire a health check at once - a thundering herd instead of the single reprobe the interval promises. State was never corrupted; the rate limit was simply defeated. | New `claim_probe(interval)` checks and updates `last_probe` atomically; only the winner probes. Verified: 32 threads racing one downed endpoint now produce exactly 1 probe. |
...
| 12 | `transport.HttpServer` | A single `requests.Session` shared across the 8-worker pool. Investigated on its merits rather than taken on faith (§7.3) and found not to be a live bug; fixed anyway with a lazily-created thread-local Session, which costs nothing and removes the question for any future server that *does* set cookies. |
...
A later audit aimed at the evidence pipeline rather than the client found three
things that made the project's own claims harder to reproduce than they should
have been. All three are now closed on both branches:
...
`test-state/` patterns were unanchored and would have swallowed any same-named
directory nested beneath `sync-demo/`; and the docstring mood was inconsistent
within single files (`"Publish ..."` next to `"Verified, idempotent store
..."`). Both fixed.
...
`lightweight-portable` from `evidence/ab_benchmark.py`. Never synthesized.
Source data: `evidence/local_metrics.json`, `evidence/docker_metrics.json`,
...
post-zstd), per operation, in KB throughout. The initial sync uploads nearly the
full 3,072 KB of the two files (no baseline to dedup against yet); the 50 KB edit
uploads the two chunks its content-defined boundaries straddled, not the whole
3 MB file; the rename uploads **zero** despite touching the full file; the no-op
pass and the delete upload nothing at all.
...
The baseline file synced to the primary in 2.1 s; with the primary genuinely
stopped, the next file reached the secondary in 1.0 s; after the primary came
back healthy, new work returned to it. The primary container is really stopped -
this is not a simulated outage.
...
shaping (never touching the host) across five loss/delay configurations: 2.1 s
clean, 6.3 s at 10 %/50 ms, 18.0 s at 10 %/100 ms, and no convergence inside the
90 s bound at either 30 %/100 ms or 20 %/200 ms. Reported as timeouts rather than
smoothed over.

`lightweight-portable`, whose adaptive concurrency window replaces this branch's
eight static workers, converged at 30 %/100 ms in the same comparison - see
[`tradeoff_analysis.md`](tradeoff_analysis.md) §3. That is a single run of a
stochastic process on both sides, so the direction is the finding rather than
the seconds.
...
landed on - **9 primary, 11 secondary**, confirming `run_once`'s per-file random
...
**Plot 8** - peak CPU per container. This branch's client peaks at **56 %**
against `lightweight-portable`'s **87 %**: zstd and BLAKE3 cost meaningfully less
per byte than `zlib` and `hashlib.blake2b`, which is what the compiled
dependencies buy on the CPU axis. The memory axis runs the other way - 52 MiB
here against 30 MiB there, since 8 static workers over chunks up to 1 MiB buffer
more at once.

![Total bytes sent](plots/09_bandwidth_comparison.png)

**Plot 9** - total client egress across both scenarios: 56.2 MB here against
29.0 MB for `lightweight-portable`. This is **not** a clean dedup-ratio
comparison, and
...
to a cause. The dedup question is answered properly below.

### 8.1 What content-defined chunking actually buys
...
The insert row is the entire justification for three compiled dependencies: a
**33× reduction** in bytes on the wire for an edit that shifts the file. The
overwrite row is the honest counterweight - fixed-size chunking did *better*
there, because these variable boundaries straddled the edit while the fixed ones
contained it. That is luck rather than a systematic advantage, and it would
reverse at a different offset. Appends are identical.
...
- **Three compiled dependencies.** `fastcdc`, `blake3` and `zstandard` are
  C/Rust extensions, so this branch cannot be installed on a host without a
  toolchain or a matching prebuilt wheel. `lightweight-portable` exists for
  exactly that case.
...
- **`MAX_CHUNK` is 1 MiB, and so is nginx's default `client_max_body_size`.**
  A maximum-size chunk of incompressible content can exceed the cap and be
  rejected with 413. Content-defined boundaries rarely land exactly at the
  maximum, so this is hit far less often here than with a fixed 1 MiB chunker -
  but `nginx.conf` raises the cap as defence in depth rather than relying on
  "rarely".
- **Manifest metadata scales with chunk count**, at ~44 bytes per chunk packed.
  Not a bug; a documented scaling boundary, whose answer is to spill the packed
  form to SQLite.
