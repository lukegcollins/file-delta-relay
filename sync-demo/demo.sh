...
ensure_venv() {
  ...
  if [ ! -x .venv/bin/python ]; then
    ...
    if command -v uv >/dev/null 2>&1; then
      ...
      uv pip install --python .venv -q -r client/requirements.txt -r server/requirements.txt -r tests/requirements.txt -r evidence/requirements.txt
    else
      python3 -m venv .venv
      .venv/bin/pip install -q -r client/requirements.txt -r server/requirements.txt -r tests/requirements.txt -r evidence/requirements.txt
    fi
  fi
}
...
full() {
  ...
  fi
  {
    ...
    echo "| Bandwidth | fixed 256 KiB chunks + dedup + zlib | $(grep -m1 'dedup:' "$happy_log" || echo 'see appendix A') |"
    ...
    echo "| Integrity | BLAKE2b verify-on-write, byte compare | $(grep -m1 'byte-identical' "$happy_log" || echo 'see appendix A') |"
    echo
