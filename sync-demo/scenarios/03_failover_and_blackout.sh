...
# Measured on this stack at CHUNK_SIZE=256 KiB: 10%/100ms converges in ~41s
# (close to the pre-portability-refactor baseline of ~50s). At the
# portability branch's earlier CHUNK_SIZE=1023 KiB, this same step did not
# reliably converge within 400s+ -- a single ~1 MiB PUT was large enough to
# meaningfully risk the read timeout under sustained loss, and each timeout
# risked cascading into an extended endpoint-unhealthy/failover-flapping
# dead-time episode. 180s budget leaves ample margin over the measured 41s
# for run-to-run variance in simulated loss. See tradeoff_analysis.md.
