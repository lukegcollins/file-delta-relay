...
class SyncServer(Protocol):
    """Define the four-call server contract sync_file/_upload are written against.
...
class CommitResult:
    ...
    server -- see the cross-server note on `known`/`ask` in sync_file below)
    so the caller can re-upload and retry.
    """
...
def chunk_manifest(path: str) -> tuple[list[Chunk], bytes]:
    """Return the content-defined chunk manifest and file hash for `path`.
...
def _retry(op: Callable[[], None], attempts: int | None = None) -> None:
    """Retry `op` with bounded, jittered backoff.

    Only network errors are retryable - a server *rejection* (bad hash,
    quota) raises straight through. Backoff is deliberately per worker:
    independent streams keep progressing and jitter desynchronizes them;
    a batch-level retry loop would reintroduce head-of-line blocking.
