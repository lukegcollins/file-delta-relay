...
class Reason(Enum):
    """Enumerate why a path needs attention; the verdict of classify()."""
...
class _StatLike(Protocol):
    """Declare the two stat fields classify() actually reads.
...
def classify(rec: "FileRec | None", st: _StatLike, now_ns: int) -> Reason | None:
    """Decide what one stat result means for one file: the decision kernel.
...
def scan(db: "StateDB", roots: list[str]) -> Iterator[Change]:
    """Sweep the roots to reconcile state; the only detector of deletions.
...
def check(db: "StateDB", paths: Iterable[str],
    ...
    """Run the kernel over candidate paths from an OS watcher: primary path.
...
def _unpack(blob: bytes) -> Manifest:
    """Decode a _pack blob back to a manifest."""
...
class StateDB:
    """Hold the SQLite state shared by the detector and the transfer path.
    ...
    def __init__(self, dbpath: str):
        """Open (creating if needed) the state database at `dbpath` in WAL mode."""
    ...
    def mark_synced(self, path: str, key: tuple[int, int], file_hash: bytes) -> None:
        """Promote the manifest to the dedup baseline once the commit succeeded.
    ...
    def mark_dirty(self, path: str) -> None:
        ...
        Keep the old dedup baseline; the path is re-chunked on the next pass.
        """
        self._c.execute(
    ...
    def prev_hashes(self, path: str) -> set[bytes]:
        """Return the chunk hashes of the last *synced* manifest.

        Those chunks are already on the server.
        """
        row = self._c.execute(
