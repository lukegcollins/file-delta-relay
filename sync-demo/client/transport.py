...
less regular and more like organic API activity. Additionally, each file
sync is assigned to a randomly chosen healthy server, spreading traffic
across endpoints while preserving per-file consistency.
"""
...
class HttpServer:
    ...
    def _session(self) -> requests.Session:
        """This thread's Session, created and configured on first use."""
    ...
    def set_preferred_endpoint(self, index: int | None) -> None:
        """Force the next requests to a specific endpoint index, or None
        to restore normal failover."""
        self.preferred_endpoint = index

    # --- endpoint selection ------------------------------------------------

    def _probe(self, ep: ServerEndpoint) -> bool:
        """Active health check: GET /api/v1/status, capped at 5 s."""
    ...
    def _pick(self, exclude: list[ServerEndpoint]) -> ServerEndpoint | None:
        """The endpoint the next request should go to, or None if none will do.
    ...
    def get_missing_chunks(self, hashes: list[bytes]) -> list[bytes]:
        """Batched set-membership query via /api/v1/collect."""
...
def run_once(db: StateDB, srv: HttpServer, roots: list[str]) -> dict[str, int]:
    """One reconciliation pass; returns {"synced": n, "deleted": m}.
