...
less regular and more like organic API activity. Each file sync also
goes to a randomly chosen healthy server, spreading traffic across
...
from single_file_transfer import AIMDController, ChunkRejected, CommitResult, RetryableError, sync_file

log = logging.getLogger("sync.client")


class ServerEndpoint:
    """One configured server URL plus what the transport has learned about it.
    ...
    def mark_success(self) -> None:
        """Record a successful request and clear any failure streak."""
        with self._lock:
            if not self.healthy:
    ...
    def mark_failure(self) -> None:
        """Count one consecutive failure; mark unhealthy at the threshold."""
        with self._lock:
            self.failures += 1
...
class HttpServer:
    """The `srv` contract over HTTP, with failover across endpoints.
    ...
    has to bound a single ~256 KiB chunk on a slow link - a put that is
    merely slow must not be aborted and re-sent from scratch. 45 s (up
    from an earlier 30 s) covers a chunk PUT retried once or twice by
    TCP itself under sustained packet loss, which is normal on a merely
    degraded link, not evidence the endpoint is actually down.
    `failure_threshold` (5, up from 3) exists for the same reason:
    `mark_failure` counts *consecutive* failures across possibly-
    concurrent requests from every in-flight uploader, so under a
    link-wide condition (loss simulated on the client's own egress, not
    a per-server outage) 2-3 unlucky requests can trip a low threshold
    within seconds even though most requests are still getting through
    -- observed directly as both endpoints flapping unhealthy/healthy/
    unhealthy in a tight loop while a degraded-but-functional link burns
    the pass's time budget on failover churn instead of steady upload
    progress (see scenarios/03_failover_and_blackout.sh's degraded-link
    step). A higher threshold means a handful of losses in a row no
    longer look indistinguishable from a genuine outage. `api_key`,
    ...
    def __init__(
        ...
        *,
        ...
        timeout: float | tuple[float, float] = (5.0, 45.0),
        ...
        # One AIMD upload-concurrency window per HttpServer instance --
        # i.e. per client process's connection to its configured servers
        # -- shared across every file run_once syncs through it, rather
        # than single_file_transfer._upload creating a fresh one per file
        # (which would reset the learned window and RTT baseline on every
        # call and never let it grow past INITIAL_NETWORK_WINDOW for a
        # workload of many small files). This does mix congestion signal
        # from whichever endpoint a given request happened to land on
        # (primary/secondary failover isn't distinguished here) -- an
        # accepted simplification, not an oversight; a per-endpoint window
        # would need this to live on ServerEndpoint instead.
        self.upload_controller = AIMDController()

    @property
    def _session(self) -> requests.Session:
        """Return this thread's Session, creating and configuring it on first use."""
        s = getattr(self._local, "session", None)
        if s is None:
            s = requests.Session()
            s.verify = self._verify
            # A concrete, identifiable User-Agent rather than requests'
            # default `python-requests/X.Y.Z` -- some corporate egress
            # proxies allowlist by client signature and drop anything that
            # looks like a bare scripting library.
            s.headers["User-Agent"] = "file-delta-relay-client/1.0"
            if self._api_key:
    ...
    def set_preferred_endpoint(self, index: int | None) -> None:
        ...
        Pass None to restore normal failover.
        """
    ...
    def _request(self, method: str, path: str, **kwargs) -> requests.Response:
        ...
        Raises RetryableError (a ConnectionError subclass, which
        single_file_transfer treats as retryable) when every endpoint has
        failed.
        """
    ...
    def get_missing_chunks(self, hashes: list[bytes]) -> list[bytes]:
        """Run a batched set-membership query via /api/v1/collect."""
    ...
    def put_chunk(self, chunk_hash: bytes, payload: bytes) -> None:
        """Store one zlib-compressed chunk at its hash via /api/v1/events/<id>.
