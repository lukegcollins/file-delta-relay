...
depends on:
  ...
  * verify-on-write: every chunk's BLAKE2b digest is recomputed server-side
    and a mismatch is rejected (409), so the store can never be poisoned by
    a corrupt or mid-upload-mutated payload;
  * commit validates references and identity: a commit naming a chunk
    ...
    and a file_hash that is not the BLAKE2b digest of the ordered chunk
    digests is rejected;
...
Endpoints (all JSON except the raw chunk body):
  ...
  They deliberately resemble common telemetry/analytics APIs.
...
def _hash(data: bytes) -> bytes:
    """Return a 32-byte digest via the standard library (matches the client)."""
...
def _check_api_key() -> None:
    """Require a matching X-API-Key header when SYNC_API_KEY is set.
...
def _db() -> sqlite3.Connection:
    """Open a fresh connection per request (sqlite objects are thread-bound)."""
...
def _chunk_path(hexhash: str) -> str:
    """Return the on-disk location of a chunk.

    Sharded by first byte so no single directory ends up with millions of
    entries.
    """
...
def _have(hexhash: str) -> bool:
    """Test set membership for the content-addressed store."""
...
def health() -> Response:
    """Report liveness for the client's failover probe and the compose healthcheck."""
...
def stats() -> Response:
    """Report chunk, file, and tombstone counts for the demo scenarios."""
...
def missing() -> Response:
    """Return the subset of the given hashes the store does not hold.
...
def put_chunk(hexhash: str) -> tuple[str, int]:
    """Store one zlib-compressed chunk, verified and idempotent.

    The body is decompressed and its BLAKE2b digest recomputed; a mismatch
    is a 409 and nothing is written. Re-putting an existing hash is a no-op
    204. The endpoint may accept query parameters like `?type=blob`
    (ignored) to appear more like an event ingestion API.
    """
    ...
    if _hash(data).hex() != hexhash:                 # verify-on-write
...
def commit() -> Response:
    ...
    try:
        expected = _hash(b"".join(bytes.fromhex(h) for h in chunk_hashes)).hex()
...
def delete() -> Response:
    ...
    Chunk data is retained: it stays a dedup baseline.
    """
    body = request.get_json(force=True) or {}
    try:
        path = body["path"]
