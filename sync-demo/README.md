...
simulation, and adds network emulation so the reliability claims are
demonstrated under a link that actually fails.

> **Branch note.** This is `lightweight-portable`: the client and server use
> nothing outside the standard library except `requests` and `flask` -
> fixed-size 256 KiB chunking, `hashlib.blake2b`, `zlib`, and an adaptive
> upload-concurrency window. Branch `main` implements the same protocol with
> `fastcdc` content-defined chunking, BLAKE3 and zstd. See
> [`tradeoff_analysis.md`](tradeoff_analysis.md) for the measured comparison.
...
├── evidence/
│   ├── local_harness.py          instrumented no-Docker pass  → local_metrics.json
│   ├── docker_harness.py         instrumented live-stack pass → docker_metrics.json
│   ├── ab_benchmark.py           docker stats sampler         → metrics_<branch>.json
│   ├── make_plots.py             renders every *.json into plots/*.png
│   ├── run_full_evidence.sh      one stack lifecycle, all of the above
│   └── publish.sh                copies results to ../evidence/<branch>/
├── plots/                        the generated evidence figures (FINAL_REPORT §8)
...
content-addressed store. Chunks are addressed by their BLAKE2b-256 digest, so
deduplication is set membership and uploads are idempotent.
...
`(LOCAL_WORKERS + MAX_NETWORK_WINDOW) × CHUNK_SIZE` - 10 × 256 KiB, or about
2.5 MB - regardless of file size.
...
it to the next one with no delay. After five consecutive failures an
endpoint is skipped until an active `GET /api/v1/status` probe says it is back,
...
The threshold is five rather than three because `mark_failure` counts
*consecutive* failures across every concurrent uploader at once. Under a
link-wide condition - loss shaped onto the client's own egress, not a server
outage - three unlucky requests in a row can trip a low threshold within
seconds while most requests are still getting through, and the client then
burns the pass flapping between two endpoints that are both fine. Five, with a
45 s read timeout, distinguishes "degraded but working" from "actually down".
...
| **Bandwidth** | Fixed 256 KiB chunking + server dedup (`/api/v1/collect`) + local pre-diff (`prev_hashes`) + per-chunk zlib level 1. Scenario 1 steps 2–3 (chunk count); integration checks 3, 4. |
...
| **Integrity** | Per-chunk BLAKE2b-256 verified server-side on write; file identity = BLAKE2b-256 of the ordered chunk hashes, recomputed and checked by the server on commit; commit validates references; a transient scan error is never mistaken for a deletion. Scenario 1 step 4; integration checks 1, 10. |
