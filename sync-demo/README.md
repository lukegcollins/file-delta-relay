...
simulation, and adds network emulation so the reliability claims are demonstrated
rather than asserted.

> **Branch note.** This is `main`: the throughput-oriented implementation, using
> `fastcdc` content-defined chunking, BLAKE3 and zstd, with a static pool of 8
> upload workers. Branch `lightweight-portable` implements the same protocol
> using only the standard library, at the cost of shift-resistant dedup. See
...
│   ├── chunking_shift_test.py    chunker-only edit-shape benchmark (§ tradeoff)
